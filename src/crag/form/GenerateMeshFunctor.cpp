...
void form::GenerateMeshFunctor::GenerateMesh(Node & node)
{
	...
	switch (num_mid_points)
	{
		...
		case 2: {
			...
			mesh.AddFace(* node.triple[TriMod(non_mid_point_index+2)].mid_point, * node.triple[TriMod(non_mid_point_index+1)].mid_point, * node.triple[non_mid_point_index].corner);
