...
namespace gl 
{
	...
	inline void BlendFunc(BlendFactor source, BlendFactor destination)
	{
		assert(destination != GL_SRC_ALPHA_SATURATE);
		GLPP_CALL(glBlendFunc(source, destination));
	}
	inline void GetBlendFunc(BlendFactor & source, BlendFactor & destination)
	{
		source = static_cast<BlendFactor>(GetInt<GL_BLEND_SRC>());
		destination = static_cast<BlendFactor>(GetInt<GL_BLEND_DST>());
	}
