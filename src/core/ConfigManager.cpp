...
bool ConfigManager::Load()
{
	...
	{
		DEBUG_MESSAGE("Failed to open config file, %s.", config_filename);
		return false;
	}
	...
	{
		DEBUG_MESSAGE("Failed to open defaults file, %s.", defaults_filename);
	...
	for (int line_num = 0; ! config_file.eof(); ++ line_num)
	{
		...
		{
			DEBUG_MESSAGE("ConfigManager: Unrecognised default parameter on line %d.", line_num);
			DEBUG_MESSAGE("ConfigManager: Program defaults will be used for remainder of values.");
			break;
		}
		...
		{
			DEBUG_MESSAGE("ConfigManager: Invalid line %d of config file.", line_num);
			DEBUG_MESSAGE("ConfigManager: Program defaults will be used for remainder of values.");
			break;
		}
...
void ConfigManager::Save()
{
	...
	if (defaults_file_ood)
	{
		...
		if (! defaults_file.is_open())
		{
			DEBUG_MESSAGE("Failed to open config defaults file, %s.", defaults_filename);
		}
	}
